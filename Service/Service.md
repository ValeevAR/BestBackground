...
# Нагрузочное тестирование внутри

Помимо прочего, в репозитории хранится скрипт для нагрузочного тестирования сервиса на машине.

Для нагрузочного тестирования внутри докер-контейнера, когда на внешней машине нет графического интерфейса или не установлены pip и python необходимо зайти в bash контейнера, как описывалось ранее:
...
Далее необходимо перейти в директорию с locustfile:

```bash
cd bestbg_app
```

Далее нужно установить locust для данной машины (он не входит в базовую процедуру установки внутри контейнера для экономии времени и места):

```bash
pip install locust
```

После завершения установки можно запускать процедуру нагрузочного тестирования без интерфейса с выводом внутри докер-контейнера:

```bash
locust --web-host 127.0.0.1 --host http://127.0.0.1:8080 --headless
```

В результате запустится тестирование, результаты которого будут интерактивно отображаться в консоли.

Для завершения тестирования и вывода финальных логов достаточно нажать Ctrl+C.

# Нагрузочное тестирование извне

Обратите внимание, что запуск в данной секции запуск locust предполагается на внешней машине с графическим интерфейсом и установленным pip и python, а не внутри Докер-контейнера. Для нагрузочного тестирования внутри докер-контейнера смотрите раздел Нагрузочное тестирование внутри.
...
Результаты нагрузочного тестирования для представленных референсных серверов представлены в файле [locust_logs.txt](locust_logs.txt).
